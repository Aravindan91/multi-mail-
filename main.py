...
try:
    with smtplib.SMTP_SSL(serveur_smtp,port_smtp,context=contexte) as server:
        ...
        for i in contact:
            ...
            with open('html-model.html','r',encoding='utf-8') as f :
               template_contenu = f.read()
               # ici on fait une photocpie de notre html 'f', 
               html_template = Environment(autoescape=True).from_string(template_contenu)
               # ici on fait une photocpie de notre html 'f', pn le met ds une envleoppe speciale MIMEAPP.. avc l
            ...
            html_final = html_template.render(sx=sx, nom=nom, prenom=prenom)
